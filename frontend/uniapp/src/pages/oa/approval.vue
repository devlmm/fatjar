<!--
  ====================================================================================
  fatjar uniapp - 审批管理 src/pages/oa/approval.vue
  ------------------------------------------------------------------------------------
  功能：
    1. 审批列表展示（标题/类型/状态/内容）
    2. 按标题 + 状态搜索
    3. 分页加载
    4. 调用接口 GET /oa/approval/page
  ====================================================================================
-->
<template>
  <view class="page">
    ...
    <view class="search-bar">
      <input v-model="query.title" placeholder="标题" class="search-input" placeholder-class="placeholder" />
    ...
    <view class="list">
      <view class="list-item" v-for="item in list" :key="item.id">
        <view class="item-main">
          ...
          <text class="item-sub">类型：{{ item.type }} | 内容：{{ item.content }}</text>
        </view>
...
export default {
  data() {
    return {
      query: { title: '', status: '', pageNum: 1, pageSize: 10 },
      list: [],
    }
  },
  onLoad() {
  ...
  methods: {
    ...
    async loadData() {
      try {
        const res = await request.get('/oa/approval/page', { data: this.query })
