<!--
  ====================================================================================
  fatjar uniapp - 凭证管理 src/pages/fico/voucher.vue
  ------------------------------------------------------------------------------------
  功能：
    1. 凭证列表展示（凭证号/标题/金额/借贷方向/期间/状态）
    2. 按凭证号 + 状态搜索
    3. 分页加载
    4. 调用接口 GET /fico/voucher/page
  ====================================================================================
-->
<template>
  <view class="page">
    ...
    <view class="search-bar">
      <input v-model="query.voucherNo" placeholder="凭证号" class="search-input" placeholder-class="placeholder" />
    ...
    <view class="list">
      <view class="list-item" v-for="item in list" :key="item.id">
        <view class="item-main">
          <text class="item-title">{{ item.voucherNo }}</text>
          <text class="item-sub">{{ item.title }} | 金额：{{ item.amount }} | {{ directionText(item.direction) }} | 期间：{{ item.period }}</text>
        </view>
...
export default {
  data() {
    return {
      query: { voucherNo: '', status: '', pageNum: 1, pageSize: 10 },
      list: [],
    }
  },
  onLoad() {
  ...
  methods: {
    ...
    async loadData() {
      try {
        const res = await request.get('/fico/voucher/page', { data: this.query })
