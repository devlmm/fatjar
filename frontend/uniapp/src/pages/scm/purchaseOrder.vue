<!--
  ====================================================================================
  fatjar uniapp - 采购订单 src/pages/scm/purchaseOrder.vue
  ------------------------------------------------------------------------------------
  功能：
    1. 采购订单列表展示（订单号/供应商/总金额/状态）
    2. 按订单号 + 状态搜索
    3. 分页加载
    4. 调用接口 GET /scm/purchase-order/page
  ====================================================================================
-->
<template>
  <view class="page">
    ...
    <view class="search-bar">
      <input v-model="query.orderNo" placeholder="订单号" class="search-input" placeholder-class="placeholder" />
    ...
    <view class="list">
      <view class="list-item" v-for="item in list" :key="item.id">
        <view class="item-main">
          ...
          <text class="item-sub">供应商：{{ item.supplierName }} | 总金额：{{ item.totalAmount }}</text>
        </view>
...
export default {
  data() {
    return {
      query: { orderNo: '', status: '', pageNum: 1, pageSize: 10 },
      list: [],
    }
  },
  onLoad() {
  ...
  methods: {
    ...
    async loadData() {
      try {
        const res = await request.get('/scm/purchase-order/page', { data: this.query })
