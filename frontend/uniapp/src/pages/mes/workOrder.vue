<!--
  ====================================================================================
  fatjar uniapp - 工单管理 src/pages/mes/workOrder.vue
  ------------------------------------------------------------------------------------
  功能：
    1. 工单列表展示（工单号/产品名称/数量/状态）
    2. 按工单号 + 状态搜索
    3. 分页加载
    4. 调用接口 GET /mes/work-order/page
  ====================================================================================
-->
<template>
  <view class="page">
    ...
    <view class="search-bar">
      <input v-model="query.workOrderNo" placeholder="工单号" class="search-input" placeholder-class="placeholder" />
    ...
    <view class="list">
      <view class="list-item" v-for="item in list" :key="item.id">
        <view class="item-main">
          <text class="item-title">{{ item.workOrderNo }}</text>
          <text class="item-sub">产品：{{ item.productName }} | 数量：{{ item.quantity }}</text>
        </view>
...
export default {
  data() {
    return {
      query: { workOrderNo: '', status: '', pageNum: 1, pageSize: 10 },
      list: [],
    }
  },
  onLoad() {
  ...
  methods: {
    ...
    async loadData() {
      try {
        const res = await request.get('/mes/work-order/page', { data: this.query })
